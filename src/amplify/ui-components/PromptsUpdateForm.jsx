...
export default function PromptsUpdateForm(props) {
  const {
    id: idProp,
    prompts,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = promptsRecord
      ? { ...initialValues, ...promptsRecord }
      : initialValues;
    setPromptText(cleanValues.PromptText);
    setActive(cleanValues.active);
    setErrors({});
  };
  const [promptsRecord, setPromptsRecord] = React.useState(prompts);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Prompts, idProp) : prompts;
      setPromptsRecord(record);
    };
    queryData();
  }, [idProp, prompts]);
  React.useEffect(resetStateValues, [promptsRecord]);
  const validations = {
    PromptText: [],
    active: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Prompts.copyOf(promptsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "PromptsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Prompt text"
